 ...
 */
function xmldb_versereminder_upgrade($oldversion=0) {
    global $DB;
    $dbman = $DB->get_manager();
    if ($oldversion < 2014071701) {
        // Define new fields to support emailing managers.
        // Define field emailrecipient to be added to versereminder to record who should receive emails.
        $table = new xmldb_table('versereminder');
        ...
        upgrade_mod_savepoint(true, 2014071701, 'versereminder');
    }

    // Add verseremindercount fields.
    if ($oldversion < 2016080301) {

        $table = new xmldb_table('versereminder');
        $field = new xmldb_field('verseremindercount', XMLDB_TYPE_INTEGER, '3', null, XMLDB_NOTNULL, null, '1');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_mod_savepoint(true, 2016080301, 'versereminder');
    }
    ...
    if ($oldversion < 2016112400) {
        ...
        // Define field thirdpartyemails to be added to versereminder to record who should receive emails.
        ...
        upgrade_mod_savepoint(true, 2016112400, 'versereminder');
    }
    // Set default value
    if ($oldversion < 2017040400) {
        ...
        upgrade_mod_savepoint(true, 2017040400, 'versereminder');
    }

    if ($oldversion < 2017102001) {
        global $CFG;
        require_once($CFG->dirroot.'/mod/versereminder/lib.php');
        ...
        $sql = "SELECT rin.*, cm.id as cmid, r.emailuser
                  FROM {versereminder_inprogress} rin
                  JOIN {course_modules} cm ON cm.instance = rin.versereminder
                  JOIN {modules} m ON m.id = cm.module
                  JOIN {versereminder} r on r.id = rin.versereminder
             LEFT JOIN {course_modules_completion} cmc ON cmc.coursemoduleid = cm.id AND cmc.userid = rin.userid
                 WHERE m.name = 'versereminder' AND cmc.id is null
        ...
        foreach ($missingprogress as $missing) {
            ...
            if (($missing->emailuser == versereminder_EMAILUSER_COMPLETION) ||
                ($missing->emailuser == versereminder_EMAILUSER_NEVER) ||
                ($missing->emailuser == versereminder_EMAILUSER_TIME && !empty($missing->emailsent))) {
                // No need to keep 'inprogress' record for later emailing
                // Delete inprogress record.
                debugging('', DEBUG_DEVELOPER) && mtrace("mode $missing->emailuser versereminderid $missing->id.
                      User marked complete, deleting inprogress record for user $missing->userid");
                $DB->delete_records('versereminder_inprogress', array('id' => $missing->id));
            } else {
                ...
                $DB->update_record('versereminder_inprogress', $updaterecord);
            }
        ...
        upgrade_mod_savepoint(true, 2017102001, 'versereminder');
    }
