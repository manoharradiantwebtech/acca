...
if (!$view) {
    // The standard view.
    echo '<table class="webex-formelements" align="center" cellpadding="5">' . "\n";

    $formelements = array(
        //get_string('description', 'webexactivity')  => $webexmeeting->intro,
        get_string('meetingpassword', 'webexactivity') => $webexmeeting->password,
    ...
    if ($recordings = $webexmeeting->get_recordings()) {
		...
		echo '<table id="recordings" class="webex-recordings-table">';
		echo '<thead>';
		echo '<tr>';
		echo '<th>' . get_string('recordings', 'webexactivity') . '</th>';
		echo '<th>' . get_string('recordingname', 'webexactivity') . '</th>';
		echo '<th>' . get_string('recordingdate', 'webexactivity') . '</th>';
		echo '<th>' . get_string('recordingdetails', 'webexactivity') . '</th>';
		echo '<th>' . get_string('editingbuttons', 'webexactivity') . '</th>';
		echo '</tr>';
		echo '</thead>';
		echo '<tbody>';
		...
		foreach ($recordings as $recording) {
			if ($recording->visible) {
				echo '<tr class="recording">';
			} else {
				// If hidden and we don't have management, then skip.
				if (!$canhost) {
					continue;
				}
				echo '<tr class="recording hiddenrecording">';
			}
			...
			echo '<td>';
			echo '<div class="play-download-buttons">';
			// Play button.
			...
			if ($candownload) {
				...
				$urlobj = new moodle_url('/mod/webexactivity/view.php', $params);
				echo $OUTPUT->action_icon($urlobj->out(false), new \pix_icon('download', 'Download', 'mod_webexactivity'), null, array('target' => '_blank'));
			}
			echo '</div>';
			echo '</td>';
			...
			// Recording name column.
			echo '<td>';
			echo '<div class="recording-name">' . $recording->name . '</div>';
			echo '</td>';
			...
			echo '<td>';
			echo '<div class="recording-date">' . userdate($recording->timecreated) . '</div>';
			echo '</td>';

	 

			// Recording length and size column.
			echo '<td>';
			echo '<div class="recording-length">';
			$params = new \stdClass();
			...
			echo get_string('recordinglength', 'webexactivity', $params);
			echo '</div>';
			echo '</td>';
			...
			if ($canhost) {
				// Editing buttons column.
				echo '<td>';
				echo '<div class="recording-actions">';
				...
				// Show/Hide button.
				$params['action'] = $recording->visible ? 'hiderecording' : 'showrecording';
				$urlobj = new moodle_url('/mod/webexactivity/view.php', $params);
				$actions['showhide'] = new action_menu_link_secondary(
					$urlobj,
					new \pix_icon('t/' . ($recording->visible ? 'hide' : 'show'), $recording->visible ? 'Hide' : 'Show'),
					get_string($recording->visible ? 'hide' : 'show')
				);
