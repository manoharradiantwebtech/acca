...
function tool_lp_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    /**
     * Start Changes
     * node - Added new node as user progress.
     * @package user profile
     * @author Radiant Web Tech
     */
    $userid = optional_param('id', 0, PARAM_INT);
    if($userid) {
        $url = new moodle_url('/local/participant/userprogress.php', array('id' => $userid));
        $node = new core_user\output\myprofile\node('miscellaneous', 'userprogress',
            get_string('userprogress', 'theme_radiant'), null, $url);
        $tree->add_node($node);
    }//End
    return true;
}
