...
/**
  * Start Changes.
  * Multi-tenant- deal with any passed company parameters so we can set the theme and other good stuff..
...
/**
  * Start Changes.
  * Multi-tenant- Set the theme if the server hostname matches one of ours..
  * @author Radiant Web Tech
  */
...
if (!empty($SESSION->has_timed_out)) {
    ...
    /**
       * Start Changes.
       * Multi-tenant- check is company url set or not.
       * @author Radiant Web Tech
       */
    if (!$hascompanybyurl) {
...
if ($frm and isset($frm->username)) {                             // Login WITH cookies
    ...
    /**
      * Start Changes.
      * Multi-tenant Company logo file setting.
      * @author Radiant Web Tech
      */
    ...
    if($current_theme->value == 'radiant') {
        if ($_SESSION['answer'] == $_POST['answer']) {
            ...
            if (empty($errormsg)) {
                if ($_SESSION['answer'] != $_POST['answer']) {
                    $errormsg = get_string("invalidcaptcha", "theme_radiant");
                    $errorcode = 3;
                }
            }
        }
...
/**
  * Start Changes.
  * Multi-tenant-  changes to display the instructions..
  * @author Radiant Web Tech
  */
