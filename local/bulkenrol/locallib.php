    ...
    if (!$emailisvalid) {
        ...
        if (!empty($error)) {
            ...
            if (!array_key_exists($email, $checkedemails->user_groups)) {
                $checkedemails->user_groups[$email] = array();
            }
                        ...
                        if (!empty($enrolinstance)) {
                            ...
                            foreach ($userstoenrol as $user) {
                                try {
                                    /**
                                     * Start Changed
                                     * Added the startdate and endate to bulk enrol user enrolment
                                     * @package bulk enrolment
                                     * @author Radiant Web Tech
                                     */
                                    $timestart = time();
                                    $enrolperiod =  $DB->get_record('enrol', array('courseid'=> $courseid, 'enrol'=>'manual'));
                                    if($enrolperiod->enrolperiod) {
                                        $timeend = $timestart + $enrolperiod->enrolperiod;
                                        $plugin->enrol_user($enrolinstance, $user->id, $roleid, $timestart, $timeend);
                                    } else {
                                        $plugin->enrol_user($enrolinstance, $user->id, $roleid, $timestart);
                                    }
                                    //End Changes
                                } catch (Exception $e) {
                                    $a = new stdClass();
